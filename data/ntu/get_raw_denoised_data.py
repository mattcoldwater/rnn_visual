...
# cp -r '.../data/ntu/raw_data/' 'root'
save_path = osp.join(root_path, 'denoised_data')
# after the program, remember to zip denoised_data, upload to '.../data/ntu/'
# !tar --use-compress-program=pigz -cf denoised_data.tgz 'root/denoised_data'
# !tar --use-compress-program=pigz -xf denoised_data.tgz -C 'root/test/'
...
def get_raw_denoised_data():
    ...
    raw_denoised_joints = [0,] * num_skes
    raw_denoised_colors = [0,] * num_skes
    frames_cnt = [0,] * num_skes
    ...
    for (idx, bodies_data) in enumerate(raw_skes_data):
        ...
        raw_denoised_joints[idx] = joints
        raw_denoised_colors[idx] = colors
        frames_cnt[idx] = num_frames
        gc.collect()

        if (idx + 1) % 1000 == 0:
            print('Time per item', (time.time()-start_time)/(idx+1))
