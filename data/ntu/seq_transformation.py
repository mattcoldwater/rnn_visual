...
### save path
# save_path = ''./'
save_path = osp.join(root_path, 'transformed_data')
...
def seq_translation(skes_joints):
    ...
    for idx, ske_joints in enumerate(skes_joints):
        ...
        ########################################
        if (idx + 1) % 10000 == 0:
            print(idx+1, ' Time per item', (time.time()-start_time)/(idx+1))
...
def align_frames(skes_joints, frames_cnt):
    """
    Not-Align all sequences with the same frame length.

    """
    print("Not-Align all sequences with the same frame length.")
    start_time = time.time()
    ##########################
    # num_skes = len(skes_joints) # skes_joints: a list
    ...
    for idx, ske_joints in enumerate(skes_joints):
        ...
        if num_bodies == 1:
            ...
            skes_joints[idx] = np.hstack((ske_joints, np.zeros_like(ske_joints)))
        else:
...
def split_dataset(skes_joints, label, performer, setup, evaluation, save_path):
    ...
    with open(x_pkl, 'wb') as f:
        pickle.dump(skes_joints[train_indices], f, pickle.HIGHEST_PROTOCOL)

    with open(y_pkl, 'wb') as f:
        pickle.dump(label[train_indices], f, pickle.HIGHEST_PROTOCOL)

    with open(valid_x_pkl, 'wb') as f:
        pickle.dump(skes_joints[val_indices], f, pickle.HIGHEST_PROTOCOL)

    with open(valid_y_pkl, 'wb') as f:
        pickle.dump(label[val_indices], f, pickle.HIGHEST_PROTOCOL)

    with open(test_x_pkl, 'wb') as f:
        pickle.dump(skes_joints[test_indices], f, pickle.HIGHEST_PROTOCOL)

    with open(test_y_pkl, 'wb') as f:
        pickle.dump(label[test_indices], f, pickle.HIGHEST_PROTOCOL)


def get_indices(performer, setup, evaluation='C-Subject'):
    
    if evaluation == 'C-Subject':  # Cross Subject (Subject IDs)
        train_ids = [1, 2, 4, 5, 8, 9, 13, 14, 15, 16, 17, 18, 19, 25, 27, 28, 31, 34, 35,
                    38, 45, 46, 47, 49, 50, 52, 53, 54, 55, 56, 57, 58, 59, 70, 74, 78,
                    80, 81, 82, 83, 84, 85, 86, 89, 91, 92, 93, 94, 95, 97, 98, 100, 103]
        test_ids = [x for x in range(1, 106+1) if x not in train_ids]
        pp = performer
    ...
    for idx in test_ids:
        temp = np.where(pp == idx)[0]  # 0-based index
        test_indices = np.hstack((test_indices, temp)).astype(np.int)

    # Get indices of training data
    for train_id in train_ids:
        temp = np.where(pp == train_id)[0]  # 0-based index
...
if __name__ == '__main__':
    ...
    setup = np.loadtxt(setup_file, dtype=np.int) # setup id: 
    ...
    skes_joints = np.asarray(skes_joints)

    evaluations = ['C-Subject', 'C-Setup']
